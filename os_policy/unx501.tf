resource "google_os_config_os_policy_assignment" "unx501" {
      ...
      resources {
        ...
        exec {
          validate {
            ...
            script = <<-EOT
              if [ ! -f /etc/rsyslog.d/91-unx501.conf ] ; then
                exit 101
              elif ! sudo stat -c %a /etc/rsyslog.d/91-unx501.conf | grep -qw '644' ; then
                exit 101
              else
                exit 100
              fi
